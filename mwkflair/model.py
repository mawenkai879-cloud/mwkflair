...
from .harma_modules import MultimodalGatedAdapter, build_harma_adapters
...
class FLAIR(nn.Module):
    ...
    def __init__(
        ...
        # === HarMA Integration: Multimodal Gated Adapter ===
        if self.use_harma:
            logging.info("Initializing HarMA Multimodal Gated Adapters...")
            # Create image adapter (processes image global features)
            self.harma_image_adapter = MultimodalGatedAdapter(
                hidden_dim=embed_dim,
                reduction_dim=harma_reduction_dim,
                num_heads=harma_num_heads,
                shared_adapter=None,
                dropout=harma_dropout,
                layer_id=0
            )
            
            # Create text adapter (processes text global features)
            self.harma_text_adapter = MultimodalGatedAdapter(
                hidden_dim=embed_dim,
                reduction_dim=harma_reduction_dim,
                num_heads=harma_num_heads,
                shared_adapter=None,
            ...
            for param in self.harma_image_adapter.parameters():
                param.requires_grad = True
            for param in self.harma_text_adapter.parameters():
                param.requires_grad = True
                
            logging.info(f"HarMA adapters initialized: reduction_dim={harma_reduction_dim}, "
                        f"num_heads={harma_num_heads}, dropout={harma_dropout}")
        else:
            self.harma_image_adapter = None
            self.harma_text_adapter = None
    ...
    def get_logits_with_harma(self, image, text):
        """
        Get logits using HarMA-enhanced global features.
        This provides an alternative to FLAIR's fine-grained retrieval by using
        cross-modal adapted global features for coarse-grained matching.
        ...
        Returns:
            Tuple of (image_logits, text_logits) using HarMA features
        """
        if not self.use_harma:
            logging.warning("HarMA is not enabled. Falling back to CLIP-style global matching.")
            return self.get_logits_as_clip(image, text)
        
        # Encode using FLAIR backbone
        global_image_token, _ = self.encode_image(image)
        global_text_token, _ = self.encode_text(text)
        ...
        # Apply HarMA adapters (self-attention)
        harma_image_features = self.harma_image_adapter(
            global_image_token, 
            context=None
        )
        harma_text_features = self.harma_text_adapter(
            global_text_token,
            context=None
        )
        
        # Normalize
        image_features = F.normalize(harma_image_features, dim=-1)
        text_features = F.normalize(harma_text_features, dim=-1)
    ...
    def forward(
        ...
        # Encode image and text using FLAIR's backbone
        global_image_token, local_image_tokens = self.encode_image(image)
        global_text_token, local_text_tokens = self.encode_text(text)
        
        # Post-process features (projection + normalization)
        ...
        # === HarMA Integration: Apply Multimodal Gated Adapters ===
        if self.use_harma and self.harma_image_adapter is not None:
            # Store original features for comparison/analysis
            flair_image_features = global_image_token
            flair_text_features = global_text_token
            
            # HarMA adapters use self-attention (no cross-modal context)
            # This is more stable and matches the original HarMA design
            harma_image_features = self.harma_image_adapter(
                global_image_token, 
                context=None  # Self-attention only
            )
            
            # Text adapter: also uses self-attention
            harma_text_features = self.harma_text_adapter(
                global_text_token,
                context=None  # Self-attention only
            )
            
            # Normalize HarMA-enhanced features for similarity computation
            harma_image_features = F.normalize(harma_image_features, dim=-1)
            harma_text_features = F.normalize(harma_text_features, dim=-1)
        else:
            # No HarMA adapters, use FLAIR features directly
            flair_image_features = global_image_token
            flair_text_features = global_text_token
            harma_image_features = None
            harma_text_features = None

        # Build output dictionary
        out_dict = {
            # FLAIR features (for fine-grained retrieval via attention pooling)
            "image_features": global_image_token,  # FLAIR global image features
            "image_tokens": local_image_tokens,     # FLAIR local image tokens
            "text_features": global_text_token,     # FLAIR global text features
            "logit_scale": self.logit_scale.exp(),
            "visual_proj": self.visual_proj,
            
            # HarMA features (for coarse-grained global matching)
            "harma_image_features": harma_image_features,  # HarMA-enhanced image features
            "harma_text_features": harma_text_features,    # HarMA-enhanced text features
            "use_harma": self.use_harma,
        }
